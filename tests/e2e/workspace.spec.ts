...
test.describe('desktop authoring workbench', () => {
  ...
  test('keeps the desktop Nexus canvas readable inside the split preview at default zoom', async ({ page }) => {
    ...
    expect(geometry.intrinsicSurfaceWidth).toBeLessThanOrEqual(geometry.availableWidth + 1)
...
Blank line above and below this final sentence.`)
    ...
    expectNear(metrics.link!.top - metrics.spoiler!.bottom, 1)
  ...
  test('searches settings by controls, validates theme imports, and exports a custom theme', async ({ page }) => {
    ...
    await (await fileChooser).setFiles({ name: 'invalid.json', mimeType: 'application/json', buffer: Buffer.from('{"tokens":{}}') })
    ...
    await page.getByRole('button', { name: 'Import theme' }).click()
    await (await fileChooser).setFiles({ name: 'valid.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(theme)) })
  ...
  test('navigates expanded settings, resets a category, and confirms destructive data reset', async ({ page }) => {
    ...
    await expect(page.getByLabel('Recovery points to keep')).toHaveValue('50')
  ...
  test('inserts colour and spoiler tools into Markdown as Nexus-compatible source', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Colour text' })).toBeVisible()
    await page.getByRole('button', { name: 'Insert coloured text' }).click()
    ...
    await page.getByLabel('Content hidden inside the spoiler').fill('Browser-tested secret')
    await page.getByRole('button', { name: 'Insert spoiler into document' }).click()
  ...
  test('keeps colour and spoiler tools working through BBCode and Visual', async ({ page }) => {
    ...
    expect(await sourceValue(page)).toContain('[color=#d97732]coloured text[/color]')
    await page.getByRole('button', { name: 'Build spoiler' }).click()
    await page.getByLabel('Content hidden inside the spoiler').fill('BBCode secret')
    await page.getByRole('button', { name: 'Insert spoiler into document' }).click()
    ...
    await expect(preview(page).locator('span[style*="color"]').last()).toContainText('coloured text')
    await page.getByRole('button', { name: 'Build spoiler' }).click()
    await page.getByLabel('Content hidden inside the spoiler').fill('Visual secret')
    await page.getByRole('button', { name: 'Insert spoiler into document' }).click()
  ...
  test('stores, inserts, and deletes reusable components and templates', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Use entire document' }).click()
    ...
    await page.getByRole('button', { name: 'Save entire document as template' }).click()
    await expect(page.getByRole('button', { name: 'Insert template QA template' })).toBeVisible()
    await page.getByRole('button', { name: 'Delete template QA template' }).click()
    await expect(page.getByRole('button', { name: 'Insert template QA template' })).toHaveCount(0)
  })
  ...
  test('configures, updates, reviews, and detaches a linked component instance', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Use entire document' }).click()
    await page.getByText('Add variables optional').click()
    await page.getByRole('button', { name: 'Add and insert variable' }).click()
    await expect(page.getByRole('button', { name: '{{version}} Insert again' })).toBeVisible()
    await page.getByLabel('Component source').fill('[b]{{version}}[/b]')
    await page.getByRole('button', { name: 'Save component' }).click()
    await page.getByRole('button', { name: 'Close tools' }).click()
    ...
    await page.getByLabel('Component source').fill('[color=#fb923c][b]{{version}}[/b][/color]')
    await page.getByRole('button', { name: 'Save changes' }).click()
    await page.getByText('Uses in this document').click()
    await page.getByText('Compare changes').click()
    ...
    await expect(page.getByRole('status')).toContainText('Linked use updated')
    await page.getByRole('button', { name: 'Close tools' }).click()
    ...
    await expect(page.getByText('Uses in this document')).toHaveCount(0)
    await page.getByRole('button', { name: 'Close tools' }).click()
  ...
  test('tracks, replaces, and cleans remote image usage', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Save URL and insert' }).click()
  ...
  test('exports and imports reusable component and template libraries', async ({ page }) => {
    ...
    await page.getByRole('complementary', { name: 'Authoring tools' }).getByRole('button', { name: 'Export', exact: true }).click()
    ...
    let fileChooser = page.waitForEvent('filechooser')
    await page.getByRole('complementary', { name: 'Authoring tools' }).getByRole('button', { name: 'Import', exact: true }).click()
    await (await fileChooser).setFiles((await componentDownload.path())!)
    await expect(page.getByRole('status')).toContainText('Imported 1 component')
    ...
    await page.getByRole('button', { name: 'Export saved templates' }).click()
    ...
    await (await fileChooser).setFiles((await templateDownload.path())!)
